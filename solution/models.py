...
class MODELSecretSauceGenerator(nn.Module):
    def __init__(self, config,
                 gpu_ids=None, use_bias=False, append_smoothers=False):
        super(MODELSecretSauceGenerator, self).__init__()
        self.config = config
        ...
        if tanh:
            self.conv_12 = nn.Sequential(nn.Conv2d(filters[5], config['output_channels'], # 2 channel output, bump and roughness
                                                   kernel_size=1, stride=1,
                                                   padding=0, bias=True),
                                         nn.Tanh())
        else:
            self.conv_12 = nn.Conv2d(filters[5], config['output_channels'], # 2 channel output, bump and roughness
                                     kernel_size=1, stride=1,
                                     padding=0, bias=True)
...
class DiscriminatorN_IN(nn.Module):
    ...
    def make_net(self, n, flt_in, flt_out=1, k=4, stride=2, bias=True):
        ...
        model.add_module('conv0',
                         self.make_block(flt_in, self.num_filters, k, stride, padding, bias, None, nn.LeakyReLU))
        ...
        for l in range(1, n):
            ...
            flt_mult = min(2 ** (l), 8)
            model.add_module('conv_%d' % (l),
                             self.make_block(self.num_filters * flt_mult_prev, self.num_filters * flt_mult,
